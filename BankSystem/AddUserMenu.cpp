...
AddUserMenu::AddUserMenu(string name, Menu* parent):
	Menu(name, parent)
{
}
...
Menu* AddUserMenu::process()
{
	...
	User* createdUser = DataBase::instance.addUser(User::REGULAR, loginData.first, loginData.second);
...
AddAdminMenu::AddAdminMenu(Menu* parent):
...
Menu* AddAdminMenu::process()
{
	...
	User* createdUser = DataBase::instance.addUser(User::ADMIN, loginData.first, loginData.second, false);
	if (createdUser == nullptr) {
		cout << "Something went wrong!\n";		
	}
	else {
